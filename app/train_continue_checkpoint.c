...
int main(int argc, char **argv) {
    if (argc < 3) {
        printf("usage: train_continue_checkpoint <model> <data> [normal] [qat]\n");
        return 1;
    }

    train_float_init();
    if (train_float_load_checkpoint(argv[1]) != 0) {
    ...
    float mid = train_float_eval_two_plane_fast();
    printf("mid eval %.4f\n", mid);
    ...
    float after = train_float_eval_two_plane_fast();
    printf("after eval %.4f\n", after);
    if (after > mid + 0.2f) {
        train_float_restore_head();
        after = mid;
        printf("restored head\n");
    }
    printf("final eval %.4f\n", after);

    if (train_float_save_checkpoint(argv[1]) != 0) {
        printf("save failed\n");
        return 1;
    }
    printf("saved %s\n", argv[1]);
