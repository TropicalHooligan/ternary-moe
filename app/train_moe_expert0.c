...
int main(int argc, char **argv) {
    if (argc < 3) {
        printf("usage: train_moe_expert0 <data> <model> [normal] [qat] [expert_thr]\n");
        return 1;
    }
    ...
    for (int e = 0; e < normal; ++e) {
        float loss = train_float_epoch_fast_expert0_trainable(thr);
        printf("expert0 normal %d loss %.4f\n", e, loss);
    }

    float eval_normal = train_float_eval_two_plane_fast_expert0();
    printf("expert0 normal eval %.4f\n", eval_normal);
    train_float_backup_head();

    for (int e = 0; e < qat; ++e) {
        float loss = train_float_qat_two_plane_epoch_fast_expert0();
        printf("expert0 qat %d loss %.4f\n", e, loss);
    }

    float eval_qat = train_float_eval_two_plane_fast_expert0();
    printf("expert0 qat eval %.4f\n", eval_qat);
    if (eval_qat > eval_normal + 0.2f) {
        train_float_restore_head();
        eval_qat = eval_normal;
        printf("restored head\n");
    }
    printf("final expert0 eval %.4f\n", eval_qat < eval_normal ? eval_qat : eval_normal);
