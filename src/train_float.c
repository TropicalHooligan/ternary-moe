...
static uint8_t head_q[VOCAB * RB1];
static uint8_t head_q2[VOCAB * RB1];
static float head_scale[VOCAB];
static float head_scale2[VOCAB];
...
void train_float_init(void) {
    ...
    int wb1 = expert_w1_bytes(DIM, HID); int wb2 = expert_w2_bytes(DIM, HID); (void)wb1; (void)wb2;
    for (int e = 0; e < E; ++e) expert_fill_identity(mw1 + e * wb1, mw2 + e * wb2, DIM, HID);
 ...
 * path that scores a context vector against the quantized two-plane
 * head, so it gets a forward declaration instead of being duplicated
 * inline at each call site. */
static void two_plane_logits_i32(int32_t *out, const int8_t *x);

static void logits_f32_to_i32(int32_t *out, const float *in, int n) {
    for (int i = 0; i < n; ++i) {
        float v = in[i];
...
int train_float_load_checkpoint(const char *path) {
    ...
    if (!f) return -1;
    if (fread(&magic, 4, 1, f) != 1 || magic != CKPT_MAGIC) { fclose(f); return -1; }
...
static void fast_relu(int8_t *y, const int8_t *x, int n) {
...
void train_float_backup_head(void) {
    for (int i = 0; i < VOCAB * DIM; ++i) head_backup[i] = head_w[i];
}

void train_float_restore_head(void) {
    for (int i = 0; i < VOCAB * DIM; ++i) head_w[i] = head_backup[i];
}
...
/* MOE_RANDOM_STAGE_MARKER */

extern void expert_fill_random(uint8_t *w1, uint8_t *w2, int dim, int hidden, int seed);

static void route_by_prev(uint8_t b) {
    int e = (int)(b % E);
    for (int i = 0; i < E; ++i) scores[i] = (i == e) ? 1.0f : 0.0f;
}

void train_float_init_random_experts(void) {
    int wb1 = expert_w1_bytes(DIM, HID); int wb2 = expert_w2_bytes(DIM, HID); (void)wb1; (void)wb2;
    for (int e = 0; e < E; ++e) expert_fill_random(mw1 + e * wb1, mw2 + e * wb2, DIM, HID, e * 7919 + 17);
}
...
/* MOE_CHECKPOINT_STAGE_MARKER */

int train_float_save_checkpoint_moe(const char *path) {
    head_quantize_two_plane(head_w, head_q, head_q2, head_scale, head_scale2, VOCAB, DIM);
    uint32_t magic = 0x544D4F32u;
    FILE *f = fopen(path, "wb");
    if (!f) return -1;
    fwrite(&magic, 4, 1, f);
    fwrite(head_w, 1, sizeof(head_w), f);
...
int train_float_load_checkpoint_moe(const char *path) {
    ...
    FILE *f = fopen(path, "rb");
    if (!f) return -1;
    if (fread(&magic, 4, 1, f) != 1 || magic != 0x544D4F32u) { fclose(f); return -1; }
    if (fread(head_w, 1, sizeof(head_w), f) != sizeof(head_w)) { fclose(f); return -1; }
    if (fread(head_q, 1, sizeof(head_q), f) != sizeof(head_q)) { fclose(f); return -1; }
    if (fread(head_q2, 1, sizeof(head_q2), f) != sizeof(head_q2)) { fclose(f); return -1; }
    if (fread(head_scale, 1, sizeof(head_scale), f) != sizeof(head_scale)) { fclose(f); return -1; }
...
/* EXPERT_TRAIN_STAGE_MARKER */

extern void expert_update_w2(uint8_t *w2, int16_t *acc2, const float *grad_y, const int8_t *h, int dim, int hidden, int threshold);
extern void expert_update_w1(uint8_t *w1, int16_t *acc1, const uint8_t *w2, const float *grad_y, const int8_t *h, const int8_t *x, int dim, int hidden, int threshold);
...
static void head_f32_grad_x_fast(float *gx, const float *grad) {
    for (int i = 0; i < DIM; ++i) gx[i] = 0.0f;
    for (int o = 0; o < VOCAB; ++o) {
        float g = grad[o];
        if (g == 0.0f) continue;
        const float *row = head_w + o * DIM;
        for (int i = 0; i < DIM; ++i) gx[i] += g * row[i];
    }
}

float train_float_epoch_fast_moe_trainable(int expert_thr) {
    if (!fast_ready) build_context_cache();
    float total = 0.0f; int count = 0; const float lr = 0.0005f; static float gx[DIM];
    for (int pos = CTX; pos < data_len; ++pos) {
        int e = (int)(data[pos - 1] % E); route_by_prev(data[pos - 1]);
        int target = data[pos];
        const int8_t *ctx = ctx_cache + pos * DIM;
        moe_forward_top1(moe_out, mw1, mw2, scores, ctx, E, DIM, HID, 0, 0, h, scratch);
        head_f32_logits(logits_f, head_w, moe_out, VOCAB, DIM);
        ...
        head_f32_train_step(head_w, grad, moe_out, VOCAB, DIM, lr);
        int wb1 = expert_w1_bytes(DIM, HID); int wb2 = expert_w2_bytes(DIM, HID); (void)wb1; (void)wb2;
        uint8_t *w1 = mw1 + e * wb1; uint8_t *w2 = mw2 + e * wb2;
        int16_t *a1 = expert_acc1 + e * HID * DIM; int16_t *a2 = expert_acc2 + e * DIM * HID;
...
/* EXPERT0_STAGE_MARKER */

static void route_first(void) {
...
float train_float_epoch_fast_expert0_trainable(int expert_thr) {
    ...
    for (int pos = CTX; pos < data_len; ++pos) {
        ...
        head_f32_train_step(head_w, grad, moe_out, VOCAB, DIM, lr);
        int wb1 = expert_w1_bytes(DIM, HID); int wb2 = expert_w2_bytes(DIM, HID); (void)wb1; (void)wb2;
        uint8_t *w1 = mw1; uint8_t *w2 = mw2;
        int16_t *a1 = expert_acc1; int16_t *a2 = expert_acc2;
        expert_update_w1(w1, a1, w2, gx, h, ctx, DIM, HID, expert_thr);
        expert_update_w2(w2, a2, gx, h, DIM, HID, expert_thr);
        total += loss;
        count++;
